...
const Event = require("../model/Job.model.js");
const User = require("../model/user.model.js");
// POST an event
router.post("/jobevent", async (req, res) => {
  const {
    companyname,
    jobtitle,
    requiredskills,
  ...
  try {
    const existingEvent = await Event.findOne({ companyname });
    if (existingEvent) {
...
// DELETE an event
router.delete("/jobdeleteEvent/:id", async (req, res) => {
  try {
...
// UPDATE an event
router.patch("/jobeditEvent/:id", async (req, res) => {
  try {
...
router.get('/jobgetevents', async (req, res, next) => {
  try {
...
router.get('/jobgetevents/:id', async (req, res, next) => {
  try {
      // Fetch all student profiles from the database
...
router.get("/jobmyevents", async (req, res) => {
  try {
